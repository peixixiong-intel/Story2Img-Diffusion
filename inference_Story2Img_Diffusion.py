...
class Story2Img_Difussion():
    ...
    def inference_generate_sample_with_condition(self, text, truncation_rate, save_root, batch_size,fast=False):
        ...
        str_cond = str(condition.replace(' ', ''))[:10]
        save_root_ = os.path.join(save_root, str_cond)
        os.makedirs(save_root_, exist_ok=True)
...
if __name__ == '__main__':
    Story2Img_Difussion = Story2Img_Difussion(config='OUTPUT/story2img_train/configs/saved_parameter.yaml', path='OUTPUT/story2img_train/checkpoint/saved_parameter.pth')
    # Story2Img_Difussion.inference_generate_sample_with_condition(text="Some giraffes are standing on the grass, they are eating in front of a forest.",truncation_rate=0.86, save_root="RESULT",batch_size=2,fast=2)  # fast is a int from 2 to 10
    Story2Img_Difussion.inference_generate_sample_with_condition(text="Some giraffes are standing on the grass, they are eating in front of a forest.",truncation_rate=0.85, save_root="RESULT",batch_size=8)
